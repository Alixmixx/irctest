...
void Server::handleList(Client* client, std::vector<std::string> arguments)
{
	...
	std::vector<std::string> list =
		arguments.empty() ? std::vector<std::string>() : split(arguments[0], ',');
	for (std::vector<Channel*>::iterator channel = _channels.begin(); channel != _channels.end();
		 channel++)
		if (shouldReplyChannel(*channel, client, list))
			client->reply(RPL_LIST, (*channel)->getName(),
						  toString((*channel)->getChannelUsers().size()), (*channel)->getTopic());
