...
void Client::reply(ReplyCode replyCode, std::string arg1, std::string arg2, std::string arg3, std::string arg4) const
{
	...
	case RPL_LUSERCLIENT:
		return reply(":There are " + arg1 + " users and " + arg2 + " invisible on " + arg3 + " servers", replyCode);
	case RPL_LUSEROP:
		return reply(arg1 + " :operator(s) online", replyCode);
	case RPL_LUSERUNKNOWN:
		return reply(arg1 + " :unknown connection(s)", replyCode);
