...
void Client::reply(ReplyCode replyCode, std::string arg1, std::string arg2, std::string arg3, std::string arg4) const
{
	switch (replyCode)
		{
		case RPL_WELCOME:
			return addInfo(1, ":Welcome to the " + std::string(NETWORKNAME) + " " + _nickname + "[!" + _username + "@" + _hostname + "]");
		case RPL_YOURHOST:
			return addInfo(2, ":Your host is " + _server->getServerName() + ", running version " + _server->getServerVersion());
		case RPL_CREATED:
			return addInfo(3, ":This server was created on " + formatTime(_server->getServerCreationTime()));
		case RPL_MYINFO:
			return addInfo(4, ":" + _server->getServerName() + " " + _server->getServerVersion() + " <available user modes> <available channel modes>");
		case RPL_ISUPPORT:
			return addInfo(5, ""); //"005 :<1-13 tokens>  :are supported by this server");
		case RPL_UMODEIS:
		...
		case RPL_WHOISUSER:
			return addInfo(311, arg1 + " ~" + arg2 + " " + arg3 + " * :" + arg4); // TODO VERIFIER
		case RPL_LIST:
